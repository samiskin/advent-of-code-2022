    ...
    for cmd in parsed {
        if cmd.starts_with("$ cd ..") {
            ...
            for i in stack.iter_mut() {
                *i += u32::from_str_radix(size, 10).unwrap();
            }
        }
    }
    let root_size = stack[0];
    while !stack.is_empty() {
        sums.push(stack.pop().unwrap());
    }

    let p1 = sums.iter().filter(|u| **u <= 100000).fold(0, |s, n| s + n);

    let remaining = 70000000 - root_size;
    let required = 30000000 - remaining;
    sums.sort();
    let p2 = sums[sums.iter().position(|u| *u >= required).unwrap()];
