#[allow(dead_code)]
mod utils;
use std::collections::HashMap;

#[allow(unused_imports)]
use crate::utils::*;

fn main() {
    // let input = _get_test_input();
    // let face_dim = 4;
    // let label_cell = |label: i32| match label {
    //     1 => (2, 0),
    //     2 => (0, 1),
    //     3 => (1, 1),
    //     4 => (2, 1),
    //     5 => (2, 2),
    //     6 => (3, 2),
    //     _ => panic!(),
    // };
    // let stitches = vec![
    //     ((4, '>'), (6, '^', true)),
    //     ((5, 'v'), (2, 'v', true)),
    // ];

    let input = _get_input();
    let face_dim = 50;
    let label_cell = |label: i32| match label {
        1 => (1, 0),
        2 => (2, 0),
        3 => (1, 1),
        4 => (0, 2),
        5 => (1, 2),
        6 => (0, 3),
        _ => panic!(),
    };
    let stitches = vec![
        ((1, '^'), (6, '<', false)),
        ((1, '<'), (4, '<', true)),
        ((2, 'v'), (3, '>', false)),
        ((2, '^'), (6, 'v', false)),
        ((2, '>'), (5, '>', true)),
        ((3, '<'), (4, '^', false)),
        ((5, 'v'), (6, '>', false)),
    ];

    // ----------- Parse Input -----------

    let parsed = input.trim_start_matches("\n").trim_end_matches("\n");
    let (grid_str, cmds_str) = parsed.split_once("\n\n").unwrap();

    #[derive(Debug)]
    enum Op {
        Move(i32),
        Turn(char),
    }
    let mut num_builder = String::new();
    let mut cmds = vec![];
    for ch in cmds_str.chars() {
        if ch.is_numeric() {
            num_builder.push(ch);
        } else {
            if !num_builder.is_empty() {
                cmds.push(Op::Move(num_builder.parse().unwrap()));
                num_builder = String::new();
            }
            cmds.push(Op::Turn(ch));
        }
    }
    if !num_builder.is_empty() {
        cmds.push(Op::Move(num_builder.parse().unwrap()));
    }

    let mut grid = Grid::from_str(grid_str);

    // ----------- Solve -----------

    fn dir_char(dir: (isize, isize)) -> char {
        match dir {
            (1, 0) => '>',
            (0, 1) => 'v',
            (-1, 0) => '<',
            (0, -1) => '^',
            _ => panic!(),
        }
    }

    fn char_dir(ch: char) -> (isize, isize) {
        match ch {
            '>' => (1, 0),
            '<' => (-1, 0),
            'v' => (0, 1),
            '^' => (0, -1),
            _ => panic!(),
        }
    }

    fn dir_num(dir: (isize, isize)) -> usize {
        match dir {
            (1, 0) => 0,  // '>'
            (0, 1) => 1,  // 'v'
            (-1, 0) => 2, // '<'
            (0, -1) => 3, // '^'
            _ => panic!(),
        }
    }
    fn num_dir(num: usize) -> (isize, isize) {
        match num {
            0 => (1, 0),  // '>'
            1 => (0, 1),  // 'v'
            2 => (-1, 0), // '<'
            3 => (0, -1), // '^'
            _ => panic!(),
        }
    }
    fn invert(num: usize) -> usize {
        let mut dir = num_dir(num);
        dir.0 *= -1;
        dir.1 *= -1;
        return dir_num(dir);
    }
    fn char_num(ch: char) -> usize {
        dir_num(char_dir(ch))
    }

    let make_graph = |grid: &Grid<char>, loop_around: bool| {
        let mut graph = HashMap::<(usize, usize), [Option<((usize, usize), usize)>; 4]>::new();

        for ((x, y), c) in grid.iter() {
            if *c != '.' {
                continue;
            }
            let mut neighbors = [None; 4];

            // if loop_around {
            let dirs: [(isize, isize); 4] = [(0, 1), (1, 0), (0, -1), (-1, 0)];
            for (dx, dy) in dirs {
                let dir = dir_num((dx, dy));
                if !loop_around {
                    let (nx, ny) = (x as isize + dx, y as isize + dy);
                    if grid.contains(nx, ny) && *grid.get(nx as usize, ny as usize) == '.' {
                        neighbors[dir] = Some(((nx as usize, ny as usize), dir));
                    }
                    continue;
                }

                let (mut nx, mut ny) = (x, y);
                loop {
                    (nx, ny) = grid.wrapped((nx as isize + dx, ny as isize + dy));
                    if *grid.get(nx as usize, ny as usize) != ' ' || !loop_around {
                        break;
                    }
                }
                if *grid.get(nx, ny) == '.' {
                    neighbors[dir_num((dx, dy))] = Some(((nx, ny), dir));
                }
            }
            graph.insert((x, y), neighbors);
        }

        return graph;
    };

    fn walk(
        grid: &mut Grid<char>,
        cmds: &Vec<Op>,
        graph: &HashMap<(usize, usize), [Option<((usize, usize), usize)>; 4]>,
    ) -> ((isize, isize), (usize, usize)) {
        let mut dir = (1, 0);
        let mut pos: (usize, usize) = (0, 0);
        while *grid.get(pos.0, pos.1) == ' ' {
            pos.0 += 1;
        }

        for cmd in cmds {
            match cmd {
                Op::Turn(c) => {
                    if *c == 'R' {
                        (dir.0, dir.1) = (-dir.1, dir.0);
                    } else if *c == 'L' {
                        (dir.0, dir.1) = (dir.1, -dir.0);
                    }
                }
                Op::Move(amt) => {
                    for _ in 0..*amt {
                        grid.set(pos.0, pos.1, dir_char(dir));
                        let neighbor = graph.get(&pos).unwrap()[dir_num(dir)];
                        if let Some(n) = neighbor {
                            pos = n.0;
                            dir = num_dir(n.1);
                        } else {
                            break;
                        }
                    }
                }
            }
        }
        return (dir, pos);
    }

    let p1_graph = make_graph(&grid, true);
    let (p1_dir, p1_pos) = walk(&mut grid.clone(), &cmds, &p1_graph);

    let mut p2_graph = make_graph(&grid, false);

    // Map of (Face label, N/S/E/W) -> edge coords in increasing order
    let mut edges = HashMap::<(i32, usize), Vec<(usize, usize)>>::new();
    for label in 1..=6 {
        let (cell_x, cell_y) = label_cell(label);
        let mut up_coords = vec![];
        let mut down_coords = vec![];
        for x in (cell_x * face_dim)..((cell_x + 1) * face_dim) {
            up_coords.push((x, cell_y * face_dim));
            down_coords.push((x, (cell_y + 1) * face_dim - 1));
        }

        let mut left_coords = vec![];
        let mut right_coords = vec![];
        for y in (cell_y * face_dim)..((cell_y + 1) * face_dim) {
            left_coords.push((cell_x * face_dim, y));
            right_coords.push(((cell_x + 1) * face_dim - 1, y));
        }
        edges.insert((label, char_num('^')), up_coords);
        edges.insert((label, char_num('v')), down_coords);
        edges.insert((label, char_num('>')), right_coords);
        edges.insert((label, char_num('<')), left_coords);
    }

    let mut stitch = |(face_a, char_a), (face_b, char_b, is_rev)| {
        let dir_a = char_num(char_a);
        let dir_b = char_num(char_b);
        let iter_a = edges.get(&(face_a, dir_a)).unwrap().iter();
        let iter_b = edges.get(&(face_b, dir_b)).unwrap().iter();
        let pair_zip: Vec<_> = if is_rev {
            iter_a.zip(iter_b.rev()).collect()
        } else {
            iter_a.zip(iter_b).collect()
        };
        for ((x1, y1), (x2, y2)) in pair_zip.iter() {
            if *grid.get(*x1, *y1) == '.' && *grid.get(*x2, *y2) == '.' {
                p2_graph.get_mut(&(*x1, *y1)).unwrap()[dir_a] = Some(((*x2, *y2), invert(dir_b)));
                p2_graph.get_mut(&(*x2, *y2)).unwrap()[dir_b] = Some(((*x1, *y1), invert(dir_a)));
            }
        }
    };
    for (a, b) in stitches {
        stitch(a, b);
    }

    let (p2_dir, p2_pos) = walk(&mut grid, &cmds, &p2_graph);
    println!("{}", grid);

    // ----------- Print -----------

    println!(
        "Part 1: {}",
        1000 * (p1_pos.1 + 1) + 4 * (p1_pos.0 + 1) + dir_num(p1_dir)
    );
    println!(
        "Part 2: {}",
        1000 * (p2_pos.1 + 1) + 4 * (p2_pos.0 + 1) + dir_num(p2_dir)
    );
}

fn _get_test_input() -> String {
    return "

        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5

"
    .to_string();
}

fn _get_input() -> String {
    return "

                                                  .#..#......#...................................................#..#............#...................#
                                                  ....#.........#.........##...............##..............#............#.##......#............#......
                                                  .............#................#.##....##.........#..#.............#...#............#................
                                                  ...........#...........#...#...........................#.#...#.#.#...#...........#................#.
                                                  ..#..................................#...............#....#...........#.#...#......#................
                                                  ......#.................#...................#.......#...............#........#......................
                                                  ...........##..............................##.......##.......#..##..........#..........#......#..#..
                                                  ..........#....#.#..........#..........#......#.#............................................#......
                                                  ........#......................#...................#....#....#........#.#...........................
                                                  .........................#....#....#.#...................#.........#.........#.........#............
                                                  .........................##.......#...#.........#.......#.....##.....#..#......#..................#.
                                                  ..........##....#.........#.##.........................#.....#...#.#......................#.........
                                                  ......#.....#............#.......#..#.....#....#.....#..........#.....#.............#..#....#....#..
                                                  #.................................#..................#.....#..#........#.................#..........
                                                  #................#....##..........................#.....#.....................#......#........#....#
                                                  .................#....##.#.#.....#...........#...#....#...#......#........#.................##......
                                                  ...................#...........................#..........#........#..#...........#.................
                                                  .#.................#....................#........#.................#....#.#..............#.........#
                                                  ..#.................#.....#...#.............#...............................#................#.#....
                                                  ......#.#........#.#......................................#..............#........#.........#.......
                                                  ...#.#.##..................#......##......#.......#.#..........................#....................
                                                  ..............#....#........####..........................#...............#............#............
                                                  ..#.............#.....###....................#....................#....#........#...................
                                                  .................#........#.#..#........................................#..............#.#..........
                                                  ....#...................................................#...#...##...#..#...........................
                                                  .......#...#...................#..............#.........#...#....#......#...............#...........
                                                  .............#...........#.#........................#...#...#.....................#.......#....#....
                                                  #............#...#..#........#...................#...........#......#..#.#.........................#
                                                  ...#...............#..#..#.............#.................#....#....#.........#.................#....
                                                  ......#...........##.#...#..#.................................##.#.#..................#..#..........
                                                  ....#........#.........#................................#.............#.....#....#..................
                                                  ......................#..........#.....#.....#.#...........#........................#............###
                                                  #...#..................#....#......#...#........................#..............#........#..#.......#
                                                  ...#............#.............#..##.#.#..#..#.#.........#.............#....#...........#.....#......
                                                  .................##......#.....................#...#....##................#..#.......#....#.........
                                                  ....#..#.........................................#....................................#...#.........
                                                  #....#...................##................#..........#..#........#..#..............#...............
                                                  .................................#..#.............#.................#...#...................#.....#.
                                                  ..#................................#.........#................#..#......................#...........
                                                  ..#..#.............##..........#......................#..............#....#...#.....#....#.....#...#
                                                  ...........##....................##..#......#...........#..............................#..#........#
                                                  ............................#.....................#.................................................
                                                  ....#..........#.#........#...................#........#.....###....................................
                                                  ...#......#..#......#..........#......#....#..............#..........................#..............
                                                  #...............#..............................#..........#......................#..................
                                                  .#...........#..............#..#.........#.......#..........#..............#........................
                                                  .................#...................#........#..#................#.......#........#................
                                                  ..............#.....##.................#...........#...#.....................#.....#....##..........
                                                  .........................#.#................#......#....#............##.......................#.....
                                                  ..#....#....................#..#...#.....#..#...........................#....##..##.......#.....#.#.
                                                  .#.....................#.....#......#.#...#....#.#
                                                  ........#...........#.........#.....#....#........
                                                  ....#...................#....#...#.......#........
                                                  ................##.#........#..#..........#..#....
                                                  ..#...............##...#......#.............#.....
                                                  .#...................#.....#.................#....
                                                  ....#.....................#.....#.....#...........
                                                  #................#...............#.##..#.#........
                                                  ..#...#....#...................................#..
                                                  ................#...............#.................
                                                  .#......................#.....#.#....#............
                                                  .......#..........#..................#..#..#..#..#
                                                  ###..###......#.#..........................#..#...
                                                  .........#.........................##.............
                                                  ..##.........#.............#......................
                                                  ...#..............#....#.#....#........#....##....
                                                  ...........................#....##.........#.#....
                                                  .............#.................#...#.#............
                                                  .............#..#....#..........................#.
                                                  .#.......#................#..##.............#....#
                                                  #..#.......#.................#........#...........
                                                  ....#...........#..............##.#....#..........
                                                  .#.................#.......#...##..........#....#.
                                                  ..................................................
                                                  ..................#.#......#......#.....#.........
                                                  .........#.....#................................#.
                                                  ..#............##.................................
                                                  ......#...#................##.......#.............
                                                  .......#...##..#..#.................#....#..#.....
                                                  ..............##..#..............#................
                                                  .....................#......#...#.................
                                                  ......................#..#.......#.....#.##.......
                                                  ..#................#...................#..........
                                                  .....##..............................#..##..#.....
                                                  .............#..#.......................#.........
                                                  ........#..................................#..#...
                                                  .....#.#........#.#................##.............
                                                  .....#.........................#..#...............
                                                  .#..#...............................#.............
                                                  .........................#.#......................
                                                  .#...............#...#.##...#................#....
                                                  .#.#..........#........#..#...............#.......
                                                  .........###..............#.......................
                                                  .....#...................#..............#.........
                                                  ..#....#.................#................#.....#.
                                                  ......#......#.............#..........#......#.###
                                                  ...................#..............#...#...........
                                                  ..................................................
                                                  #........#.....##.........................#.......
                                                  ..#....#.......#................................#.
..##.................#....................##................#.....#.#.#........#.............#...#..
................#.........#................#........##..............................................
.......................#.##.................#.........#...........#.##...........................#..
.......#.....#.............#..#....#..................................#......#..#............#.....#
............................#.........................##......#............###..###.......#.........
....#......#...........................................#.#................................#...#.....
....#..........#........#..........#.........#........................##...#..............#.....#...
#.............##.....##..........#....###...#..........#..........#........#..............#...#.....
......#...............#......#.........#.....#.#....#......#................#........#......#.......
........................#.#...........#....#..#.........#............#.#....#.....................#.
.......................................#.....#..#...........#.........#...............#...#.........
.................#........#.........#...##...#.....#..........#.....#...............................
.......#........#..#..#..#......#........#.#........#.#...#........#.........#........##.#..........
...#..#..#.......##.............................#....#.#......#.....................................
.......#.#................................#.........#....#.....#....#.....#................#........
.....#..............##..#........#.........#..................#..........#....#.....................
............#.....#.................#........##...#.................#.#...........#.................
..............#.............##...........#...#.....#.#..........#..........................#........
#....##.....................#........##....#..................#.....................................
#.............................#..#..............#...................#............#..................
.....#....#.#........#.........................#...#................#.#....................#..#.....
....................#.............#..........#....#........#...#...#................................
.........................#...##...#.#...............#........#............#.........#.#..##.........
...#........#.##.........#.....#...#..............#.......#..#...#..........#...#.......#........#..
...#........#...##............#........#.............................##........##.........#.#....#..
.....##.#......#..#.............................................#..#........###.#....#..............
.................#...#.....#......#.............#.....#....................#.............#..........
.....#.......................#.#....##..........#..#......#....................#....#........#.....#
....#....................................#..............#...........#............#..#........#.#....
................#.....#....##.....#...#..#....#.......##............#.##.#.......................#..
.#...................................#.#..................#...#........#......#.....................
...........#.................#........#..#......#..................###...........##..#..............
.....................#.........#.#.......#..................................#....##.........#.......
..................#......#.....#.......#..........#............#..#...#.........#...#.......#.......
................#.................#..#.............................#..#........#....#..#............
.........................#.........................##............#..........##........#.............
...............##.......#........#.........................#.#......#.#.........#...................
...#........#......#...................#.##........#.#...............##.#...........................
.....#.#.........#.........#.#....#...........#.#.............#................#.#......#......#....
#.............#.....#.....###...........................#......................................#....
.#..#........#......#......#..#....#......#.#................#..#.#.........#..............#......#.
....#......#.......................#...#....#.............#......##.........................#...#...
.........#............#...#...#..........#.#.........#............................##....#.........#.
...........................#...........#.......#..#....................#..................#.....##..
......#....#.#.##.....#..#..........##.....#....#.........#......#......#..........#..#.....#.#.....
.........#.........................#....#.........................#..........#.#..............#.#...
................#................#..#...............................................................
..........#.........#..................#......................##..#..#........#...............#.....
.....#........#..#.........#...............................#...#.........................#..........
....#..#......#...#.......#..#.#.#.............#.........##..#.........#...................#.....##.
.....#.................#..........................
#..#.........#....#...................#...........
..#......#....#......#............................
..#...............................#...............
....#.#...#.........................#.#.........#.
#..............##...#..#.#.........#........#.....
..............#.......................###.........
#.#...#..............#...........#...........#.#..
..............................#...................
........................#.#...#....#..............
.......#...........................#..#...........
...................#......#.....#....#.#.........#
............##........................##.#.......#
.....#................#..............##...........
..............#........#...###................#...
..#....................#.......................#..
#..#.#......#......#.........#............#.......
......................#......#....................
....#....#..#....#............#....#..............
....##...........#.......##......#......#..#..#...
.#..........##...................................#
..................#..#...........................#
...............#............................#.....
...#.......#................#.#...#........#.....#
.......##......#...#...........#...........#......
..........................#......#..#.............
.........#.......#..........#.....................
.#.............#...........#....#.................
.....#....#................#..........#...........
...#.....................#.....#..................
.......#.............#.....#.#.........#..........
...#...........#..#.................#..........##.
....##.......................#.........#...#......
............#........#.........#.......##.#..#....
..............#...........................#....###
....#........#.#....#......#.#..#.........#.......
....#....................#.........#.........#....
.........................................##.......
........#....#..#.#..#.........#...#.........#....
.............................#..#...##........#...
...#.......#.......#...............#..............
......................................#......#....
.....#..................#.........................
#.....#.....#..#..................................
..#.....#..#...#................#.....#..........#
.....................#............#...............
.......#...................................#.....#
............#.#..#..#...#.........#...............
..##..........#...#..#........#..............#....
....#.....#..............................#........



"
    .to_string();
}

